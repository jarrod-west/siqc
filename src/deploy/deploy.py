...
from mypy_boto3_cloudformation.type_defs import ParameterTypeDef, ValidateTemplateOutputTypeDef
import re

from src.shared.clients.cloudformation import CloudformationClient
from src.shared.clients.connect import ConnectClient
from src.shared.utils import InstanceConfig, Parameters, StackConfig, MAIN_STACK_CONFIG, read_parameters

CAMELCASE_SPLIT_REGEX = r"([A-Z])"

def create_stack_parameters(client: CloudformationClient, instance_config: InstanceConfig, template: str) -> list[ParameterTypeDef]:
  ...
  for parameter in parsed_template["Parameters"]:
    ...
    *name_tokens, attribute = re.sub(CAMELCASE_SPLIT_REGEX, r" \1", parameter["ParameterKey"]).split()
...
def deploy_stack(client: CloudformationClient, stack_config: StackConfig, instance_config: InstanceConfig) -> None:
...
def deploy() -> None:
  ...
  phone_numbers = connect_client.get_phone_number_summaries([parameters["PrivateNumber"], parameters["PublicNumber"]])
